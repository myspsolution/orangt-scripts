...
while [[ "$CLIENT_CODE" == "UNDEFINED" ]]; do
  ...
  printf "${BLD}3 to 12 lowercase alphanumeric${STD}, examples: ${YLW}meta${STD}, ${YLW}microsoft${STD}, ${YLW}sps${STD} : "
  read CODE_INPUT
...
echo -e "This will ${BLD}generate a random secure password${STD} for MariaDB (MySQL) ${YLW}root${STD} user,"
...
if [[ "${SERVER_ENVIRONMENT}" == *dev* ]]; then
  export DB_NAME="db_${CLIENT_CODE}_dev"
else
  export DB_NAME="db_${CLIENT_CODE}_prod"
fi

export DB_USER=$(generate_user_string)
export DB_PASSWORD_ROOT=$(openssl rand -base64 32)
...
echo -e "${YLW}sudo apt install mariadb-server -y${STD}"
...
if [ "$?" -ne 0 ]; then
  printf "\n"
  printf "${RED}Error restarting mariadb (mysql) service after replacing config${STD}\n"
  printf "\n"
  exit 1
fi
...
printf "db sa/superadmin : root (do not use in .env)\n"
printf "db sa password   : ${DB_PASSWORD_ROOT}\n"
